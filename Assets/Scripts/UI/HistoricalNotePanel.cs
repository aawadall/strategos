// HistoricalNotePanel.cs
// #462: read-only historical commentary from Scenario.HistoricalNotes (pause nested).
...
{
    public sealed class HistoricalNotePanel : MonoBehaviour
    {
        ...
        {
            var root = CreateRect("HistoricalNotePanel", host);
            Stretch(root);
            ...
            card.sizeDelta = new Vector2(560, 360);
            ...
            var title = CreateTmp("Title", card, "HISTORICAL NOTE", 18, FontStyles.Bold);
            title.color = Theme.Ink;
            title.alignment = TextAlignmentOptions.Center;
            title.gameObject.AddComponent<LayoutElement>().preferredHeight = 32;

            var note = CreateTmp("Note", card,
                "Authored commentary · not an AAR score · sources in Research/historical",
            ...
            scrollHost.gameObject.AddComponent<LayoutElement>().preferredHeight = 220;
        ...
        public void Bind(Scenario scenario) => _scenario = scenario;

        public bool HasNotes
        {
            get
            {
                if (_scenario?.HistoricalNotes == null) return false;
                foreach (var n in _scenario.HistoricalNotes)
                    if (!string.IsNullOrWhiteSpace(n.Text)) return true;
                return false;
            }
        }
        ...
        private void Refresh()
        {
            ...
            if (_scenario?.HistoricalNotes == null || _scenario.HistoricalNotes.Count == 0)
            {
                _body.text = "(no historical notes on this scenario)";
                return;
            }
            ...
            foreach (var n in _scenario.HistoricalNotes)
            {
                ...
                sb.Append(n.When == HistoricalNoteWhen.Outcome ? "OUTCOME" : "BRIEFING");
                sb.Append("\n\n").Append(n.Text.Trim()).Append("\n\n");
            }
            _body.text = sb.Length == 0 ? "(no historical notes on this scenario)" : sb.ToString();
        }
    }
}
