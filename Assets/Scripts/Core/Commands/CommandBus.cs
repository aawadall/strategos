...
{
    /// <summary>
    /// A subscriber to the command topic.
    ///
    ...
    /// </summary>
    public sealed class CommandSubscriber
    {
        public string Name;
        public int Order;
    ...
    {
        private readonly List<CommandSubscriber> _subscribers = new();
        ...
        private List<Command> _inbox = new();
        private List<Command> _delivering = new();

        private bool _dispatching;

        /// <summary>Commands awaiting delivery at the next step.</summary>
        public int PendingCount => _inbox.Count;

        /// <summary>Total commands delivered since construction. Diagnostic only.</summary>
        public int DeliveredCount { get; private set; }

        public void Subscribe(string name, int order, Action<Command> handler)
        {
            if (handler == null) return;
            _subscribers.Add(new CommandSubscriber { Name = name, Order = order, OnCommand = handler });
        ...
        /// <summary>
        /// Queues a command for delivery at the next step.
        ///
        ...
        /// </summary>
        public void Publish(in Command command) => _inbox.Add(command);

        /// <summary>
        /// Delivers everything published before this step, to every subscriber in order.
        ///
        /// Commands published during this call go to the next step's inbox.
        /// </summary>
        public void Deliver()
        {
            if (_dispatching)
                throw new InvalidOperationException(
                    "CommandBus.Deliver re-entered. A subscriber must publish, not deliver.");
            ...
            try
            {
                // Command order first, subscriber order within each command: every subscriber
                // sees command N before any subscriber sees N+1. The alternative — each
                // subscriber draining the whole batch — makes a subscriber's view of ordering
                // depend on its position in the list.
                for (int c = 0; c < _delivering.Count; c++)
                {
                    var command = _delivering[c];
                    for (int s = 0; s < _subscribers.Count; s++)
                        _subscribers[s].OnCommand(command);
                    DeliveredCount++;
                }
            }
            finally
        ...
        /// <summary>Drops undelivered commands. For resetting between replays.</summary>
        public void Reset()
        {
            _inbox.Clear();
            _delivering.Clear();
